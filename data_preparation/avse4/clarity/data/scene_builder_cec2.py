...
def select_random_interferer(
    ...
    while interferer_not_found:
        ...
        # if not filtered_interferer_group:
            # log.error(f"No suitable interferer found for dataset {dataset} and required samples")
            # log.error(f"No suitable interferer found for this noise category, selecting another category")
    ...
    #     #when noises are shorter than scene, select a random noise from group
    #     # interferer = random.choice(interferer_group) #to choose an interferer even if its shorter
    #     raise ValueError(
...
def get_random_interferer_offset(interferer: dict, required_samples: int) -> int:
    ...
    if latest_start < 0:
        ...
        # # update to AVSEC data:
        # latest_start = required_samples - masker_nsamples
...
class SceneBuilder:
    ...
    def add_listener_details_to_scene(
        ...
        for scene in tqdm(self.scenes):
            listener = {}
            listener["rotation"] = 0 # generate_rotation(
            #     scene,
